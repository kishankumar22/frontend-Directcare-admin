...
export default function SubscriptionsPage() {
    ...
    if (subscriptions.length > 0) {
      const filtered = subscriptions.filter((subscription) => {
        ...
        const matchesFrequency =
          frequencyFilter === "all" ||
          subscription.frequency === parseInt(frequencyFilter);
  ...
  const handlePause = async (id: string) => {
    setActionLoading(id);
    try {
      const response = await subscriptionsService.pause(id);
      if (response.data?.success) {
        toast.success("⏸️ Subscription paused successfully!");
        await fetchSubscriptions();
      }
    } catch (error: any) {
  ...
  const handleResume = async (id: string) => {
    setActionLoading(id);
    try {
      const response = await subscriptionsService.resume(id);
      if (response.data?.success) {
        toast.success("▶️ Subscription resumed successfully!");
        await fetchSubscriptions();
      }
    } catch (error: any) {
  ...
  const handleSkip = async (id: string) => {
    setActionLoading(id);
    try {
      const response = await subscriptionsService.skip(id);
      if (response.data?.success) {
        toast.success("⏭️ Next delivery skipped!");
        await fetchSubscriptions();
      }
    } catch (error: any) {
  ...
  const toggleSelectAll = () => {
    if (selectedSubscriptions.length === currentData.length) {
      setSelectedSubscriptions([]);
    } else {
      setSelectedSubscriptions(currentData.map((s) => s.id));
    }
  };
  ...
  const hasActiveFilters =
    statusFilter !== "all" || frequencyFilter !== "all" || searchTerm.trim() !== "";
      ...
      <div className="mx-auto space-y-4">
        ...
        <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-2xl p-6">
          ...
          <div className="space-y-4 mb-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <h2 className="text-2xl font-bold text-white">All Subscriptions</h2>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {frequencyFilter !== "all" && (
                  <span className="px-2 py-1 bg-purple-500/10 border border-purple-500/30 rounded-md text-purple-400 text-xs font-medium flex items-center gap-1">
                    Frequency: {getFrequencyBadge(parseInt(frequencyFilter))}
                    <button
                      ...
                      <td className="py-4 px-4">
                        {subscription.nextDeliveryDate ? (
                          <div>
                            ...
                            <p className="text-xs text-slate-500">
                              {subscription.totalDeliveries} deliveries
                            </p>
                          </div>
                        ) : (
        ...
        {/* Pagination */}
        {totalPages > 1 && !loadingSubscriptions && (
          <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-2xl p-4">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              <div className="text-sm text-slate-400">
                Total: {totalItems} subscriptions
              </div>
            </div>
          </div>
        )}
                ...
                <div className="bg-slate-800/30 p-4 rounded-xl border border-slate-700">
                  ...
                  <p className="text-slate-300 text-sm">
                    ...
                    <span className="font-medium text-white">
                      {cancellingSubscription.shippingFullName}
                    </span>
