              ...
              {order.pharmacyResponses && order.pharmacyResponses.length > 0 ? (
                <div className="space-y-3">
                  {Array.from(new Set(order.pharmacyResponses.map((r) => r.productName))).map((productName) => {
                    const productResponses = order.pharmacyResponses!.filter((r) => r.productName === productName);
                    const topLevel = productResponses.filter((r) => !r.parentOptionId);

                    const renderRow = (
                      r: (typeof productResponses)[number],
                      depth: number
                    ): JSX.Element => {
                      const optionVal = r.answerText || '—';
                      const isYes = optionVal.toLowerCase() === 'yes';
                      const isNo = optionVal.toLowerCase() === 'no';
                      // Short values (Yes/No/a picked option label) read fine as a pill; longer
                      // free-text answers get their own labelled block instead, so the answer
                      // never has to fight the question for space on one line.
                      const isShortAnswer = optionVal.length <= 24;
                      // Follow-ups are their own response row now — find any whose ParentOptionId
                      // matches THIS row's chosen option, and nest them beneath it.
                      const children = r.selectedOptionId
                        ? productResponses.filter((c) => c.parentOptionId === r.selectedOptionId)
                        : [];

                      return (
                        <div key={r.questionId} className={depth > 0 ? 'mt-2 ml-3 pl-3 border-l-2 border-violet-300 dark:border-violet-500/40' : ''}>
                          <div className={`rounded-lg px-3 py-2.5 border ${
                              depth > 0
                                ? 'bg-violet-50/60 dark:bg-violet-500/[0.06] border-violet-200 dark:border-violet-500/20'
                                : 'bg-slate-50 dark:bg-slate-800/60 border-slate-200 dark:border-slate-700/40'
                            }`}>
                            {depth > 0 && (
                              <div className="flex items-center gap-1 mb-1.5 text-[10px] font-bold text-violet-500 dark:text-violet-400 uppercase tracking-wide">
                                <CornerDownRight className="w-3 h-3" />
                                Follow-up question
                              </div>
                            )}

                            <p className="text-slate-700 dark:text-slate-300 text-xs font-medium whitespace-pre-line">
                              {r.questionText}
                            </p>

                            <div className="mt-2 flex items-start gap-2">
                              <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide shrink-0 mt-1">
                                Answer
                              </span>
                              {isShortAnswer ? (
                                <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${isYes
                                ...
                                <p className="flex-1 text-xs text-slate-800 dark:text-slate-200 bg-white dark:bg-slate-900/60 rounded-md px-2.5 py-1.5 border border-slate-200 dark:border-slate-700 whitespace-pre-line">
                                  {optionVal}
                                </p>
                              )}
                            </div>
