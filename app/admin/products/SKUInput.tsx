    ...
    <div>
      ...
      <div className="relative">
        <input
          ...
          placeholder={isVariableProduct ? "Blank (Not required for variable products)" : "PROD-001"}
        ...
        <div className="absolute right-3 top-1/2 -translate-y-1/2">
          {!isVariableProduct && checking && <span className="text-yellow-400 animate-pulse">...</span>}
          {!isVariableProduct && !checking && error && <span className="text-red-500">❌</span>}
          {!isVariableProduct && !checking && !error && value.length >= 3 && <span className="text-green-500">✔</span>}
        </div>
      </div>

      {!isVariableProduct && error && <p className="text-red-400 text-xs mt-1">{error}</p>}

      {!isVariableProduct && !error && value.length >= 3 && !checking && (
