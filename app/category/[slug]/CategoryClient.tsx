            ...
            <Card className="shadow-sm">
              <CardContent className="p-4">
                ...
                {allSubCategories.length > 0 && (
                  <div className="border-b border-gray-100 pb-5 mb-3">
                    <h3 className="font-bold text-sm text-gray-900mb-2  border-gray-100">Subcategories</h3>
                    <div className="space-y-1 max-h-56 overflow-y-auto custom-scrollbar pr-2 pt-1">
                      {[...allSubCategories]
                        .filter((sub) => sub.productCount > 0 && sub.name.trim() !== "-" && sub.name.trim() !== "—")
                        .sort((a, b) => a.name.localeCompare(b.name))
                        .map((sub) => (
                          <label
                            key={sub.id}
                            className="flex items-center gap-2.5 cursor-pointer hover:bg-gray-50/80 px-2 py-1 rounded-lg transition group"
                          >
                            <input
                              type="checkbox"
                ...
                <div className="border-b border-gray-100 pb-5 ">
                  <h3 className="font-bold text-sm text-gray-900 mb-2.5  border-b border-gray-100">
                    Brand
                  </h3>
                  <div className="space-y-1 max-h-64 overflow-y-auto pr-2 custom-scrollbar pt-1">
                ...
                <div className="border-b border-gray-100 pb-2">
                  <h3 className="font-bold text-sm text-gray-900 mb-2.5 pb-1.5 border-b border-gray-100">
                    Price Range
                  </h3>
                  <div className="pt-2">
                    {minPrice < maxPrice && (
                    ...
                    {[4, 3, 2, 1, 0].map((rating) => (
                      ...
                      >
                        <input
                          type="radio"
                          name="rating"
                          className="w-4 h-4 text-[#445D41] focus:ring-[#445D41] focus:ring-2 border-gray-300"
                          checked={minRating === rating}
                          onChange={() => handleRatingChange(rating)}
                        />
                        <div className="flex items-center gap-2">
                          <Star className="h-4 w-4 fill-yellow-400 text-yellow-400" />
                          <span className="text-sm text-gray-700 group-hover:text-[#445D41] transition">
                            {rating > 0
                              ? `${rating}+ Stars`
                              : "All Ratings"}
                          </span>
              ...
              <div className="lg:hidden fixed inset-0 z-50 flex">
                ...
                <div className="relative bg-white w-[78vw] max-w-xs h-full flex flex-col shadow-2xl">
                  ...
                  <div className="overflow-y-auto flex-1 px-5 py-4 space-y-6">
                    ...
                    {allSubCategories.length > 0 && (
                      <div className="border-b border-gray-100 pb-5">
                        <div className="flex items-center justify-between mb-2.5 pb-1.5 border-b border-gray-100">
                    ...
                    {brands.length > 0 && (
                      <div className="border-b border-gray-100 pb-5">
                        <div className="flex items-center justify-between mb-2.5 pb-1.5 border-b border-gray-100">
                          ...
                          <button className="text-xs text-[#445D41] font-semibold hover:underline" onClick={() => setSelectedBrands([])}>Clear</button>
                        </div>
                        <div className="space-y-1 max-h-48 overflow-y-auto pr-2 custom-scrollbar pt-1">
                          {availableBrands.map((brand) => (
                            <label
                              key={brand.id}
                              className="flex items-center gap-2.5 cursor-pointer hover:bg-gray-50/80 px-2 py-1 rounded-lg transition group"
                            >
                              <input
                                ...
                                onChange={(e) => handleBrandChange(brand.id, e.target.checked)}
                              />
                              <span className="text-sm text-gray-700 truncate group-hover:text-[#445D41] transition">{brand.name}</span>
                            </label>
                          ))}
                        </div>
                      </div>
